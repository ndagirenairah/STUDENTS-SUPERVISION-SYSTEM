...
export const tasks = pgTable('tasks', {
  id: text('id').primaryKey(),
  studentId: text('student_id').notNull().references(() => students.id),
  supervisorId: text('supervisor_id').notNull().references(() => supervisors.id),
  title: text('title').notNull(),
  description: text('description').notNull(),
  priority: text('priority', { enum: ['low', 'medium', 'high'] }).notNull(),
  deadline: timestamp('deadline').notNull(),
  expectedDeliverables: text('expected_deliverables').notNull(),
  status: text('status', { enum: ['pending', 'in_progress', 'completed', 'approved', 'rejected'] }).default('pending').notNull(),
  createdAt: timestamp('created_at').defaultNow().notNull(),
...
export const workEvidence = pgTable('work_evidence', {
  ...
  studentId: text('student_id').notNull().references(() => students.id),
  evidenceType: text('evidence_type', { enum: ['screenshot', 'github_link', 'document', 'pdf', 'source_code', 'video', 'git_commit'] }).notNull(),
  evidenceUrl: text('evidence_url'),
  description: text('description'),
  submittedAt: timestamp('submitted_at').defaultNow().notNull(),
  status: text('status', { enum: ['submitted', 'approved', 'rejected'] }).default('submitted').notNull(),
...
export const productivityScores = pgTable('productivity_scores', {
  ...
  riskReasons: jsonb('risk_reasons'), // array of strings
});
...
export const randomChecks = pgTable('random_checks', {
  ...
  studentId: text('student_id').notNull().references(() => students.id),
  supervisorId: text('supervisor_id').notNull().references(() => supervisors.id),
  checkType: text('check_type', { enum: ['screenshot_request', 'video_call', 'progress_question'] }).notNull(),
  question: text('question'),
  requestedAt: timestamp('requested_at').defaultNow().notNull(),
  respondedAt: timestamp('responded_at'),
  response: text('response'),
  responseTime: integer('response_time'), // in minutes
  status: text('status', { enum: ['pending', 'responded', 'missed'] }).default('pending').notNull(),
});
