...
import {
  ...
  Wifi, User, Calendar, Target, LayoutDashboard, ClipboardList, LogIn
} from 'lucide-react';
...
export default function StudentDashboardClient({ data, logoutAction }: { data: DashboardData; logoutAction: () => void }) {
  ...
  const navItems = [
    ...
    { id: 'checkin', label: 'Check In/Out', icon: <LogIn className="w-4 h-4" /> },
  ...
  return (
    <DashboardShell
      ...
      subtitle={`${data.user.name} • ${data.student.studentId}`}
      accent="blue"
      navItems={navItems}
      activeId={activeTab}
